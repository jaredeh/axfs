...
void RBTreeCreate(rb_red_blk_tree* newTree,
  ...
  /*  for information on nild and root */
  temp=newTree->nild= nild;
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= nild;
  temp->parent=temp->left=temp->right=newTree->nild;
  temp->key=0;
  temp->red=0;
}
...
/*           root and nild pointers, and the node to rotate on. */
/**/
/*  OUTPUT:  None */
/**/
...
void LeftRotate(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  /*  nild to avoid checking for nild.  However this introduces a */
  /*  very subtle bug because sometimes this function modifies */
  /*  the parent pointer of nild.  This can be a problem if a */
  /*  function which calls LeftRotate also uses the nild sentinel */
  /*  and expects the nild sentinel's parent pointer to be unchanged */
  /*  after calling this function.  For example, when RBDeleteFixUP */
  /*  calls LeftRotate it expects the parent pointer of nild to be */
  /*  unchanged. */

  y=x->right;
  x->right=y->left;

  if (y->left != nild) y->left->parent=x; /* used to use sentinel here */
  /* and do an unconditional assignment instead of testing for nild */
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not red in LeftRotate");
#endif
}
...
void RightRotate(rb_red_blk_tree* tree, rb_red_blk_node* y) {
  ...
  if (nild != x->right)  x->right->parent=y; /*used to use sentinel here */
  /* and do an unconditional assignment instead of testing for nild */
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not red in RightRotate");
#endif
}
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not red in TreeInsertHelp");
#endif
}
...
void RBTreeInsert(rb_red_blk_node * x, rb_red_blk_tree* tree, void* key, void* info) {
  rb_red_blk_node * y;
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not red in RBTreeInsert");
...
rb_red_blk_node* TreeSuccessor(rb_red_blk_tree* tree,rb_red_blk_node* x) { 
  ...
  if (nild != (y = x->right)) { /* assignment to y is intentional */
    while(y->left != nild) { /* returns the minium of the right subtree of x */
      y=y->left;
    }
    return(y);
  } else {
    y=x->parent;
    while(x == y->right) { /* sentinel used instead of checking for nild */
      x=y;
      y=y->parent;
    }
    if (y == root) return(nild);
    return(y);
  }
}
...
rb_red_blk_node* TreePredecessor(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (nild != (y = x->left)) { /* assignment to y is intentional */
    while(y->right != nild) { /* returns the maximum of the left subtree of x */
      y=y->right;
    }
    return(y);
  } else {
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not black in RBDeleteFixUp");
#endif
}
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  y= ((z->left == nild) || (z->right == nild)) ? z : TreeSuccessor(tree,z);
  x= (y->left == nild) ? y->right : y->left;
  ...
  if (y != z) { /* y should not be nild in this case */

#ifdef DEBUG_ASSERT
    Assert( (y!=tree->nild),"y is nild in RBDelete\n");
#endif
...
#ifdef DEBUG_ASSERT
  Assert(!tree->nild->red,"nild not black in RBDelete");
#endif
}
...
stk_stack* RBEnumerate(rb_red_blk_tree* tree, void* low, void* high) {
  ...
  while ( (lastBest != nild) && (1 != tree->Compare(low,lastBest->key))) {
